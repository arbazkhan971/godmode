#!/usr/bin/env bash
# Godmode Installer for Codex (OpenAI)
...
if [ ! -d "$GODMODE_ROOT/.codex" ]; then
    echo "Error: cannot find .codex/ directory in $GODMODE_ROOT"
    exit 1
fi

echo "Godmode installer for Codex"
...
if [ -f "$TARGET_DIR/AGENTS.md" ]; then
    if ! cmp -s "$GODMODE_ROOT/AGENTS.md" "$TARGET_DIR/AGENTS.md"; then
        ...
        echo "[done] Updated AGENTS.md (source was newer)"
    else
...
# ---------------------------------------------------------------------------
# 2. Copy .codex/ directory (config.toml + agents/*.toml)
# ---------------------------------------------------------------------------
...
if [ -d "$CODEX_DIR" ]; then
    ...
    cp -r "$GODMODE_ROOT/.codex" "$CODEX_DIR"
    echo "[done] Copied .codex/ directory (config.toml + 7 agent definitions)"
fi

# Always sync config.toml so re-runs pick up changes
if [ -f "$GODMODE_ROOT/.codex/config.toml" ]; then
    if ! cmp -s "$GODMODE_ROOT/.codex/config.toml" "$CODEX_DIR/config.toml"; then
        cp "$GODMODE_ROOT/.codex/config.toml" "$CODEX_DIR/config.toml"
        echo "[done] Updated .codex/config.toml (source was newer)"
    else
        echo "[skip] .codex/config.toml already up to date"
    fi
fi

# Always sync all agent TOMLs so re-runs pick up new or changed definitions
mkdir -p "$CODEX_DIR/agents"
UPDATED_AGENTS=0
for src_toml in "$GODMODE_ROOT/.codex/agents/"*.toml; do
    dst_toml="$CODEX_DIR/agents/$(basename "$src_toml")"
    if ! cmp -s "$src_toml" "$dst_toml"; then
        cp "$src_toml" "$dst_toml"
        UPDATED_AGENTS=$((UPDATED_AGENTS + 1))
    fi
done
SOURCE_COUNT=$(ls "$GODMODE_ROOT/.codex/agents/"*.toml 2>/dev/null | wc -l | tr -d ' ')
if [ "$UPDATED_AGENTS" -gt 0 ]; then
    echo "[done] Synced .codex/agents/ ($UPDATED_AGENTS of $SOURCE_COUNT agent definitions updated)"
else
    echo "[skip] .codex/agents/ all $SOURCE_COUNT definitions up to date"
fi

# ---------------------------------------------------------------------------
# 3. Create .godmode/ with config.yaml (stack auto-detection)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 4. Symlink skills, agents, and commands into target (if not present)
# ---------------------------------------------------------------------------
...
echo "  Codex:     $CODEX_DIR/config.toml"
echo "  Agents:    $CODEX_DIR/agents/"
echo ""
echo "Get started:"
echo "  codex \"Run /godmode:setup\"         # configure project settings"
echo "  codex \"Run /godmode:optimize\"      # autonomous performance iteration"
echo "  codex \"Run /godmode:build\"         # TDD build workflow"
echo "  codex \"Run /godmode:secure\"        # security audit"
