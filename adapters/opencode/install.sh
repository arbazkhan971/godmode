#!/usr/bin/env bash
# Godmode Installer for OpenCode
...
echo "Godmode installer for OpenCode"
...
# ---------------------------------------------------------------------------
# 3. Create OpenCode plugin manifest
# ---------------------------------------------------------------------------
...
if [ -f "$PLUGIN_FILE" ]; then
    echo "[skip] .opencode/plugins/godmode/plugin.json already exists"
else
    cp "$SCRIPT_DIR/plugin.json" "$PLUGIN_FILE"
    echo "[done] Installed plugin.json to .opencode/plugins/godmode/"
fi

# ---------------------------------------------------------------------------
# 4. Symlink skills, agents, and commands into plugin directory (if not present)
# ---------------------------------------------------------------------------

for dir in skills agents commands; do
    LINK_PATH="$PLUGIN_DIR/$dir"
