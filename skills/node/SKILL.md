...
- **Do NOT use `cluster.fork()` in containers.** Containers should run one process — let the orchestrator (Kubernetes, ECS) handle scaling.
...
EVENT LOOP LAG AUDIT:
  ...
  Step 3: Profile with clinic.js
    npx clinic doctor -- node server.js  (flamegraph)
    npx clinic bubbleprof -- node server.js  (async bottlenecks)

  Step 4: Worker thread offloading
    Single task: new Worker(path, { workerData })
    Repeated tasks: Piscina pool (maxThreads: 4, idleTimeout: 30s)

  TARGETS: p99 < 50ms (API) | p99 < 20ms (realtime) | zero sync I/O in request path
```
