...
```
Agent 1 (worktree: legacy-assess): Full codebase health assessment (read-only)
Agent 2 (worktree: legacy-tests): Characterization tests for top 10 critical modules
Agent 3 (worktree: legacy-deps): Patch vulns, replace deprecated packages
Agent 4 (worktree: legacy-cleanup): Dead code removal, god class extraction
MERGE ORDER: assess (report only) -> tests -> deps -> cleanup
```
...
```
After EACH modernization step (dependency upgrade, dead code removal, test addition):
  1. MEASURE: Run full test suite — do all existing tests still pass?
  2. COMPARE: Is the metric improved? (coverage up, vuln count down, dead code removed)
  3. DECIDE:
     - KEEP if: test suite passes AND target metric improved AND no runtime errors in staging
     - DISCARD if: test suite fails OR dependency upgrade introduces breaking changes OR dead code removal causes runtime errors
  4. COMMIT kept changes in dedicated commits (one concern per commit). Revert discarded changes immediately.
```
...
STOP when ANY of these are true:
  ...
  - Characterization tests cover critical paths (top 10 most-called functions)
  - Deprecated/EOL dependencies replaced
  - User explicitly requests stop

DO NOT STOP just because:
  - Test coverage has not reached 80% (incremental progress is valid; coverage is a long-term goal)
  - Some dead code remains flagged as "needs runtime verification" (that requires production observation)
```
