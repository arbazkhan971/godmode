...
```
PHASES: env_detection → dependency_installation → command_validation → config_integrity → smoke_test → developer_readiness
FOR EACH phase: detect/install/validate → report grade (A-F) → fix failures (max 3 retries) → next phase
FINAL: all required phases PASS → READY | any required FAIL → BLOCKED | recommended FAIL → READY WITH WARNINGS
```
